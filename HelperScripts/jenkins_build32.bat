...
mkdir build
cd build
cmake -DINCLUDE_QT_WALLET=TRUE -G "Visual Studio 12" ../nameshares || exit /b 28
msbuild.exe /M:%NUMBER_OF_PROCESSORS% /p:Configuration=RelWithDebinfo /p:Platform=Win32 /target:rebuild /v:diag NameShares.sln || exit /b 30
